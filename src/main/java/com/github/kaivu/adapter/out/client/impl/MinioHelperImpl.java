...
public class MinioHelperImpl implements MinioHelper {
    ...
    // The MinIO SDK (OkHttp-based) blocks on socket I/O without holding a monitor across the
    // call, so it's safe to run on virtual threads: cheap, unbounded, no pool size to tune —
    // unlike a fixed worker pool, which would need capacity planning for concurrent uploads.
    ...
    public Uni<InputStream> getObject(String bucketName, String objectName, long offset, long length) {
        return Uni.createFrom()
                .<InputStream>item(Unchecked.supplier(() -> {
                    try {
                        log.debug(
                ...
                // MinioClient's SDK calls are blocking I/O; offload off the Vert.x event loop.
                .runSubscriptionOn(VIRTUAL_THREAD_EXECUTOR);
    }
